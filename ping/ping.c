/*************************************************************************
    ...
    > Created Time: 2016年04月09日 星期六 02时56分07秒
 ************************************************************************/
...
int main(int argc,char *argv[])
{
	...
	printf("ping %s (%s) : %d bytes of data.\n",argv[1],
				inet_ntoa(to.sin_addr),(int)ICMP_SIZE);
	...
	printf("%d packets transmitted,%d received,%%%dpacket loss",
...
/*
	...
	*len
*/
USHORT ckeckSum(USHORT *addr,int len)
{
	UINT sum = 0;
	while(len > 1)
	{
...
void pack(struct icmp* icmp, int seq)
{
	...
	icmp->checksum =ckeckSum((USHORT *)icmp,ICMP_SIZE);
}
